...
\begin{itemize}
    \item \textbf{Shared-English}: phonemes that are in both the English and Spanish dictionaries, evaluated on English speech.
    \item \textbf{Shared-Spanish}: phonemes that are in both the English and Spanish dictionaries, evaluated on Spanish speech.
...
In the next step, we extend this analysis to French and Polish. For these languages, we first build a single reference inventory that reflects everything the model could have seen during pretraining. We do this by taking the union of the English and Spanish pretraining phoneme sets, which gives us one combined set containing all phonemes that appear in either English or Spanish pretraining data. This is different from the shared and language‑unique groups defined above, which separate phonemes into “in both languages” and “in only one language”. Here, we only care whether a phoneme was present in at least one of the pretraining languages.
...
\begin{itemize}
    \item  \textbf{Overlap with En/Es (Fr)}: phonemes that appear both in the combined English+Spanish inventory and in the French inventory, evaluated on French speech.
    \item  \textbf{Overlap with En/Es (Pl)}: phonemes that appear both in the combined English+Spanish inventory and in the Polish inventory, evaluated on Polish speech.
\end{itemize}
...
As seen in the Section \ref{ed_subsec:lang_membership_groups}, by grouping phonemes only into shared vs non‑shared sets we are comparing groups that differ greatly in how many total phonemes they contain. To analyse phoneme accuracy more precisely, we can look to dividing them into manner of articulation (MoA) classes. MoA classes provide widely used articulatory categories for grouping phonemes across languages \cite{ladefoged_johnson_phonetics_textbook,ipa1999} \thoughts{these are perplexity citations needs adding to bibliography}. The total number of phonemes in each of these categories is lower than the shared/non‑shared sets, giving more precision. The phoneme inventories are also more comparable in size between languages, allowing for more balanced cross-lingual comparison. In addition, they allow analysis of some language‑specific categories that may have been obscured in the previous groupings. 

While most MoA classes follow standard articulatory categories, some of the groups in our experiments require additional explanation.\thoughts{Check ipa book to see how they classify and maybe make a reference here} We have separated diphthongs from monophthongs in the vowel category because they only occur in English in our data. Dipthongs are sounds formed by combining two vowels, so separating them may  also give some insight into coarticulation in mixed-language pretraining. Closures correspond to the \texttt{sil} token, which signifies silence in MFA, and are therefore treated as their own group rather than merged. 

The Tap \& Trill class is present in English, Spanish and Polish. However, unlike English and Polish, Spanish has more than one phoneme in this group and has far more examples of both \textipa{r} and  \textipa{R} in the training data as seen in Appendix \ref{app_sec:chapter_5_appendix} Table \ref{app_tab:phoneme_counts_per_language_train_10h_1}. While this category is not language-specific, it is heavily weighted towards Spanish, which may affect the final accuracy of phonemes in this category. Due to this, we separated out phonemes from the wider sonorants group into vowels, nasals, approximants and Taps \& Trills. This allows us to observe the language-specific behaviour of all these groups separately. The number of occurrences of every available phoneme across the training data for all four languages tested can be found in Appendix \ref{app_sec:chapter_5_appendix}.\thoughts{Double check the sonorants thing with delphine}
...
For MLS Spanish finetuning, a similar gradual change appears in the opposite direction. CER decreases consistently as we move from English high-resource pretraining towards Spanish high-resource pretraining. The highest CER is on the 800/100 model, followed by slightly lower CER on the English monolingual model, and the lowest CER on the 100/800 and Spanish monolingual models. The lowest and highest Spanish CER values therefore occur at 100/800 and 800/100 rather than at the monolingual models, unlike English. However, both languages follow the same broader high-resource pattern: CER is lowest when pretraining is high-resource and matched to the finetuning language, and highest when pretraining is high-resource and mismatched.


\hl{This section needs editing!!}
Overall, these results show that ASR performance on matched-language finetuning data closely follows our high-resource pretraining conditions: CER is lowest when the pretraining language matches the finetuning language and has the most hours, and highest when pretraining is high-resource but fully mismatched. At the same time, all finetuning data in this subsection come from the same corpora that were used during pretraining, so we cannot yet tell whether these patterns reflect corpus-specific memorisation, language-specific phonetic information, or a combination of both. To separate these effects and test whether the same trends hold beyond seen pretraining corpora, we next examine finetuning on languages and datasets that were not used during pretraining.

In the next subsection, we analyse how these pretrained models behave when finetuned on mismatched or unseen languages to test whether the advantages of matched high-resource pretraining extend beyond the pretraining corpora


\hl{next paragraph is old and needs removing/replacing.}
...
\end{figure}
\thoughts{need to regeneerate with number of phonemes in each group}
...
\nomenclature[K]{High-Resource Pretraining: }{Pretraining on a single dominant language or a strongly skewed mixture, referring to the following models in chapter \ref{chapter5}: 960 / 0, 800 / 100, 100 / 800, 0 / 920.}
