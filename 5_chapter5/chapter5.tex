...
These patterns reflect both pretraining exposure and probe data for this MoA class. Taps and trills are low‑resource in English pretraining, but their counts increase with Spanish data. As seen in Table~\ref{ed_tab:phoneme_token_class_counts_m}, Taps have 0.4M to 1.6M phoneme tokens in pretraining and trills have 0 to 0.6M tokens for English and Spanish monolingual models respectively. Both classes can be considered low resource across all models. In the probe validation sets, Spanish also has the most examples for these phonemes with 17431 \textipa{R} in taps and 5879 \textipa{r} for trills. This is compared to very few English taps with 290 \textipa{R}, 155 \textipa{r\super j} and 5 \textipa{\~R} and moderate Polish trills with 2403 \textipa{r} tokens and only 12 \textipa{r\super j}. This combination explains why Spanish taps and trills have higher accuracy than the English taps and Polish trills, despite the low-resource exposure in pretraining. Spanish also has only a single Tap phoneme, \textipa{R}, and a single Trill phoneme, \textipa{r}. With fewer distinct tokens to average across, phoneme accuracy in these groups may be more sensitive to anomalous results. With fewer phonemes to average across accuracy may be more sensitive to anomalous results. This could account for the fluctuations in accuracy and the less clear gradual increase in accuracy across mixtures when compared to previous results.\thoughts{This could all go in the discussion section almost certainly}
...
\hl{the below will be part of the analysis later}

Fricatives are a mid‑resource\thoughts{mid-resource needs to be added to nomenclature} class in pretraining ranging between 4.4M to 5.1M phoneme tokens. Spanish has more tokens in this class than English. Here English and Spanish show some matched-language increase in phoneme accuracy, most evident in the monolingual models. However, both unseen languages show little correlation between high accuracy and pretraining mixture, whilst also showing high accuracy overall. Nasals are also a mid‑resource class in pretraining, with token counts ranging from 3.3M to 3.7M. English and Polish nasals show some gradual decrease in accuracy on high-resource Spanish models. Despite the Spanish pretraining data having more examples of nasals in pretraining, Spanish nasal accuracy fluctuates across models showing little correlation with matched-language pretraining. French nasals do not appear to show correlation with high phoneme accuracy and either language, with little variance across languages. 

\hl{Need to highlight next that these mid-resource classes are both uncorrelated to pretraining data and show a wide range of accuracies. In discussion I would link this to previous findings that suggest that nassals are generally well represented and that it is surprising that the accuracy could be so low, especially for french. How do I link this here to weighted averages though?}
