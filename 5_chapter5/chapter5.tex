...
To test this idea directly for the unseen languages, we examine French and Polish phonemes that overlap with only one of the pretraining languages. Four sets of phonemes can be formed by taking phonemes that are exclusive to two languages at a time, one pretraining language and one unseen testing language, as detailed in Section~\ref{ed_subsec:lang_membership_groups}. We expect each group to perform best on the matching monolingual model and to show a gradual shift in accuracy towards the mismatched monolingual model, as seen in previous experiments.

Figure~\ref{ed_fig:cross_overlap_fre_pol} shows mean phoneme accuracy across mixtures for these four overlap groups, French-English, French-Spanish, Polish-English, Polish-Spanish. We do not observe a clear gradual shift in accuracy for any of the groups, in some cases balanced mixed-language pretraining even produces the highest accuracy. French-English and Polish-English have mean accuracies of 48.92\% and 52.27\% respectively. In contrast, the French-Spanish and Polish-Spanish groups are at 62.31\% and 64.98\% mean accuracies. Thus, for both French and Polish, phonemes that exist only in Spanish and the respective unseen language have higher average accuracy than phonemes that exist only in English and the unseen language.
...
The results in Section~\ref{ed_sec:shared_vs_not_shared} show us that shared phonemes have a higher average accuracy than unique phonemes for both languages. Simultaeneously we see that all groups show a gradual shift in accuracy, accuracy is low on the monolingual and high-resource mismatched language models but incrementally increases as more matched-language data is added to pretraining, with the highest accuracy on high-resource or monolingual matched-language models.\thoughts{very long sentence, has been explained before} These results suggest that the models may be learning some joint features across languages. However, given the gradual shifts apparent in all matched-language results and the higher accuracy of the shared phoneme groups, the models also appear to be learning language-specific features.\thoughts{some repetition still but a little more additional information.}

Next we test phonemes spoken in unseen languages, French and Polish, by dividing them into Unseen-Shared groups and Unseen-Unique groups. French-Shared and Polish shared are phonemes spoken in the respective languages but that overlap with the English or Spanish phoneme inventories. The French-Unique and Polish-Unique groups contain phonemes that do not appear in  the English or Spanish. Both shared groups have a higher average accuracy across all models when compared to their language's unique group. This appears to show that enough features have been learned by the SSL ASR models for phonemes realised in a unseen language to have an advantage over completely unseen phonemes. It is also notable that none of the groups have clearly higher accuracy on either high-resource English models or high-resource Spanish models. This shows that unseen languages benefit less from monolingual pretraining. Since any language-specific features to English or Spanish are less likely to appear in an unseen, unrelated language, only shared features \cite{li2023quantitative, XLSR-53}\thoughts{need to cover \cite{li2023quantitative} in the background section } can be used to recognise French and Polish. \thoughts{needs rewording and rethinking, xls-r paper talks about cross-lingual representations, I need to read it again, but it probably doesn't justify them}

Something, notable about the phoneme groups for both seen and unseen languages is the variety in phoneme inventory size for each group. English-Shared and Spanish-Shared have an inventory size of 26 phonemes, while English-Unique has 54 and Spanish-Unique has 10. For the unseen languages, French-Shared and Polish-Shared have an inventory of 35 phonemes whereas French-Unique has only 10 and Polish-Unique has 19. We might expect smaller inventory sizes to be more susceptible to the values of a single phoneme, since one outlier phoneme with an extreme accuracy, close to 0\% or 100\%, will have more impact on the average. We might also expect less consistent behaviour for groups with small phoneme inventories. It is therefore notable that despite the large difference in inventory sizes between Spanish-Unique and English-Unique their behaviour is still comparable. Both classes have a clear gradual shift between matched and mismatched models and both groups have low accuracy relative to their Shared group couterparts. This could imply that the behaviour is consistent across groups, regardless of how many phonemes are included in an inventory. 

French-Unique does however, have notably low accuracy even when compared to another Polish-Unique with an average accuracy of 45.78\%. Examining the French-Unique, we find that the phonemes included are: \textipa{\~A,  O, \~O, \o, \~E, ts, \oe, 4, K, y}. While \textipa{ts} is very low-resource, as outlined in section \ref{DESIGN_SECTION_HL_RESOURCE}\info{update reference to section}, the others in this group would be high or very high-resource, so a single phoneme is unlikely to affect the overall behaviour. Having mostly high-resource phonemes in test also implies that the behaviour exhibited by French-Unique is consistent with the large shared and unique groups for unseen languages. It is also notable that the phonemes in French-Unique are very varied, containing phonemes from multiple articulation groups. If French were to perform badly in one MoA group this could also affect the final outcome, possibly averaging out any accuracy increase for either pretraining language. \thoughts{bnot sure if the info in the paragraph is very useful}

The phonem inventory for Spanish-Unique is also small and, on examination, only 6 of the 9 possible phonemes in the MFA dictionary are found in the test data. The phonemes in Spanish-Unique are:\textipa{x, r, \textbardotlessj J, G, B, J}. Here we have two low-resource phonemes, and the rest are high or very high-resource. Two of six phonemes being low-resource may add some variability and could explain lower accuracy of Spanish unique on the 600/300 model when compared to 800/100 model. However, with four high-resource groups and similar overall behaviour to the English-Unique group, we cannot make any assertions about the phoneme inventory in this group. Again, we see a small group with a wide variety of phonemes from multiple MoA groups. \thoughts{maybe these two paragraphs could be shortened and combined.}

Finally, in this section, we analyse the behaviour of phonemes found in English-Unique and Spanish-Unique when spoken in the unseen languages French and Polish. To analyse whether phonemes have higher accuracy on models pretrained with more examples of that phoneme we look at the English-Unique and Spanish-Unique groups. Phonemes from these groups will only exist in models that include the matched-language and will exist in much higher numbers on high-resource and monolingual models. For phonemes that exist in only one pretraining language but also an unseen language we can test how much an unseen phoneme benefits from exposure in pretraining. The results in Figure \ref{ed_fig:cross_overlap_fre_pol} show us that phonemes in the French-Spanish-Shared and Polish-Spanish-Shared groups have higher accuracy for every model than the French and Polish English-Shared groups. We also see that Spanish-Shared phonemes do not have higher accuracy on high-resource Spanish models. Neither do English-Shared phonemes have higher accuracy on high-resource English models. These results suggest that unseen phonemes do not benefit directly from representation in pretraining.

However, these groups have significantly smaller phoneme inventories when compared to the previous groups. Both French-Spanish and Polish-Spanish only have four phonemes in their phoneme inventories; French-English has ten and Polish-English has nine. 
