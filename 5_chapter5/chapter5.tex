...
\hl{This whole section needs rewriting to be consitsent with the updated results}
...
%\hl{THIS COULD START THE NEXT SUBSECTION Need to highlight next that these mid-resource classes are both uncorrelated to pretraining data and show a wide range of accuracies. In discussion I would link this to previous findings that suggest that nassals are generally well represented and that it is surprising that the accuracy could be so low, especially for french. How do I link this here to weighted averages though?}
\iffalse
\begin{itemize}
...
\begin{itemize}
    \item We need to transition into the shared non shared section
    \item describe in detail the size of the groups phoneme inventories
    \item I should also describe the datasets and why I chose these datasets and languages
    \item mention that not all of the phoneme categories are taken from a source but listed based on what MFA includes
    
    
    \item Describe the behaviour of the groups, with more references to coarticulation and realisations
    
    
    \item Comment on why this is problematic, I probably also shoudl list the phonemes in at least the Spanish-unique group.
    \item I should comment on the very different sizes of English-unique and Spanish-unique, but how they still seem to have similar behaviour.
    \item Then address the French and Polish shared/unique
    \item I should look at the phoneme inventories of these groups, although even the unique groups are >10 each, so maybe not all the specific phonemes.
    \item Should I talk about average accuracies of shared unseen groups compared to shared seen
\end{itemize}
...
Spanish taps and trills are also shown in literature to have much variation. Different realisations of trills can be correlated to multiple factors in Spanish speakers. For example, where and how many occlusions are included in a trill can predict the gender and dialect of a speaker \cite{henriksen2023sociophonetic}. Similarly to Polish taps and trills can also in some cases be conisdered a minimal pair. When stressing or emphasising a word or syllable a spanish speaker will be more likely to pronounce a trill. However, some cases\thoughts{this is actually about trill weakening and I'm not confident that I understand it well enough to say that Spanish trills can become taps.} 
...
\newpage
\section{Future Work}
\hl{There is an iffalsed section here with my fufuture work plans}
\iffalse
