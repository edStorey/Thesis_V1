...
\begin{figure}[h]
    ...
    %\caption{Phoneme accuracy taken at layer 7 of each of our models for English and Spanish data. The phonemes are split into groups of phoneme that are either shared between Spanish and English languages or phonemes that are unique to either English or unique to Spanish. \hl{needs a rewrite!!}}
...
\iffalse
In this subsection, we first determine whether phoneme representations reflect the matched-language advantages and related-language transfer patterns observed previously in CER.\thoughts{come back to this to see whether we did actually address this} Second, we test whether exposure to MFA phoneme tokens during pretraining predicts phoneme accuracy. We address this by comparing two types of groups. Language-shared and language-overlap groups appear across most pretraining models, so they have high exposure overall. Language-unique groups have the highest exposure in high-resource matched-language models, so they vary strongly with pretraining balance. All phoneme groups are defined in Section~\ref{ed_subsec:lang_membership_groups}.\thoughts{could be cut to save space}
\fi
