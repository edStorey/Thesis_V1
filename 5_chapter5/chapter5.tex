...
\fi
With this information so far, we can see some gaps in our knowledge for SSL ASR. For example, what happens when the ratios of unrelated languages in a multilingual SSL ASR model are not imbalanced? Do languages rely on a different source of pretraining data when English is not the most abundant? \thoughts{dunno if I should keep this in} Are individual phonemes learned as well when the pretraining data is multilingual compared to when it is monolingual? This chapter will answer these questions.\summary{chapter questions} \hl{DOI WE NEED ANYTHING AFTER THIS / I could end it here}
