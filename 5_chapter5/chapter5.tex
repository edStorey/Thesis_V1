...
\bottomrule
\end{tabular}
\caption{Phoneme token counts (in millions, rounded to one decimal) grouped by phoneme class and model. For more precision, counts rounded to the closest thousand are provided in Appendix \ref{app_tab:phoneme_token_class_counts_k}. \hl{Tap \& Trills needs changing}}
...
\begin{figure}%[t]
    ...
    \caption{\hl{Taps and Trills are all included in this graph. UPDATE THIS CAPTION!!}}
    \label{ed_fig:moa_class_vowel}
\end{figure}

\hl{PLACEHOLDER TEXT IGNORE THIS}

Previously, in Figure 2, French–Spanish and Polish–Spanish overlapping sets showed higher average accuracy than the corresponding French-English and Polish-English groups. English has a larger and more diverse vowel inventory, and vowels are also the highest‑resource MoA class in pretraining, Table 2. Despite this, Spanish Vowels consistently have higher accuracy than vowels spoken in all other languages, even on the English monolingual model. Since the French–Spanish and Polish–Spanish sets contained several Spanish vowels in their small inventories, this may explain their high accuracy. The decline in English vowel accuracy across mixtures shows a matched-language correlation to pretraining, but the French and Polish vowel results do not. Next, therefore, we evaluate a MoA class with less exposure in pretraining.
