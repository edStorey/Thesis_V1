...
1. Introduce MoA results and briefly restate the three hypotheses (data quantity, seen-language trade-off, unseen-language impact).

2. Present vowel results across languages, highlighting matched-language effects for English, stability for Spanish, and mixed-pretraining benefits for Polish.

3. Present stop and nasal results as robust, high-resource classes that transfer well and are relatively insensitive to exact language balance.

4. Present fricatives (and briefly affricates) as lower-accuracy, more mixture-sensitive classes that depend more on matched pretraining and inventory differences.

5. Present rhotics (tap/trill or combined) as a strongly Spanish-skewed class showing clear matched-language benefits and trade-offs for unseen languages.

6. Briefly note approximants and (if kept) diphthongs as intermediate cases that support, but do not change, the main patterns.

7. Summarise how MoA patterns connect back to shared/unique and cross-overlap results, arguing that class type and data quantity largely determine how pretraining balance affects transfer.

8. Make comparison of weighted averages then lead on to final subsection
...
\begin{itemize}
    \item Perplexity analysis thoughts 
    \item \textbf{Vowels}: Claims accuracy is high adn Fre/Pol curves track English
    \item I disagree Polish is quite different, Spanish shows clear preference for Spanish and French is not that averagely high
    \item "more vowel examples in pretraining (from any language) help vowels in unseen languages" This would suggest that they all did best on Spanish monolignual which has the most, they do not do best on Spa monolingual
    \item barchart supports these ideasm it uses phoneme inventory to justify the more vowel examples line
    \item overall argument is the higher number of examples makes English high-resource models better for recognition. I would disagree with this reading

    \item \textbf{Approximants}: data quantity effect plus a mild English advantage, but approximants are less fragile than fricatives and less skewed than diphthongs; balanced mixtures work nearly as well as monolingual for all languages, including unseen ones
    \item Again disagree, there is a clear spanish advantage for spanish and what is it talking about with the mixed and balanced models

    \item \textbf{Nasals} : "Themes: nasals exemplify a universal, well‑represented class where total examples matter more than which language they come from; balanced and high‑resource mixed pretraining reliably helps all languages."
    \item I don't disagree completely, this class has almost the same number of examples across all models and the phoneme tokens overlap ~75\% across all languages 
    \item although it doesn't mention that Spanish has higher accuracy on monolingual English than Spanish, which maybe should be noted.
\end{itemize}
...
\end{tabular}
\caption{Number of time-aligned phone intervals (in millions, rounded to one decimal) identified by the Montreal Forced Aligner in the pretraining data, grouped by phoneme class and model. More precise token counts, rounded to the nearest thousand, are provided elsewhere.}
%\caption{Phoneme token counts (in millions, rounded to one decimal) grouped by phoneme class and model. For more precision, counts rounded to the closest thousand are provided in Appendix \ref{app_tab:phoneme_token_class_counts_k}. hl{IF THIS STAYS HERE IT NEEDS ADDRESSING IN TEXT!!}}

\label{ed_tab:phoneme_token_class_counts_m}
\end{table}

We now analyse phoneme accuracy by manner-of-articulation (MoA) to understand how pretraining language balance interacts with different phonetic classes. In this section, we highlight several specific classes. We first analyse the vowel and stop classes, which are highly represented in both pretraining languages. We then examine taps, trills, and diphthongs. These classes are either present, or heavily concentrated, in only a single pretraining language. We consider all of these classes in matched-language, mismatched-language, related-language, seen-language, and unseen-language settings. The languages used for analyses are again English, Spanish French and Polish. These results will show how much phoneme accuracy relates to phoneme representation in pretraining and explore the relationship between phoneme representation and articulation.\thoughts{Revisit this opening pargraph when I have written the whole section.}
\iffalse
\begin{figure}[t]
    \centering
    ...
    \caption{Mean vowel phoneme accuracy at layer~7 for English, Spanish, French, and Polish across all pretraining mixtures. Each panel shows accuracy for vowel phonemes in a given language for different English and Spanish pretraining mixtures.}
...
\begin{figure}%[h]
    \centering
    \includegraphics[width=\linewidth]{5_chapter5/figures/weighted_accuracy/weighted_unweighted_vowel.pdf}
    \caption{Mean vowel accuracy across English and Spanish pretraining mixtures. The left panel shows the unweighted average across phoneme categories within each language. The right panel shows the same averages, but weighted by the number of phoneme interval counts per phoneme category in the test data.}
    \label{ed_fig:moa_class_vowel_weighted}
\end{figure}
\fi
...
\hl{WRITE NEXT TAPS AND TRILLS SECTION NEXT FIRST DRAFT DONE IN PERPLEXITY}
...
\begin{figure}%[p]
  \centering
  \includegraphics[width=0.85\textwidth]{5_chapter5/figures/standard_deviation/vowel.pdf}
  \caption{Vowels \hl{Change!}}
  \label{fig:lab1}
  \centering
  \includegraphics[width=0.85\textwidth]{5_chapter5/figures/standard_deviation/fricative.pdf}
  \caption{Fricative \hl{Change!}}
  \label{fig:lab2}
  \centering
  \includegraphics[width=0.85\textwidth]{5_chapter5/figures/standard_deviation/nasal.pdf}
  \caption{Nasal \hl{Change!}}
  \label{fig:lab3}
\end{figure}



\hl{New section ends here}
