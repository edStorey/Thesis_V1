...
\subsection{Datasets}\label{ed_subsec:datsets}
...
\centering
    (CER\_on\_monolingual\_Spanish) - (CER\_on\_monolingual\_Enlgish)
\end{equation}
\newline
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{5_chapter5/figures/sharedEngSpa.pdf}
...
Figure \ref{ed_fig:phonemes_shared_not_shared} \thoughts{I need to introduce the fact that we are now using probing} shows the phoneme accuracy of four different groups of phonemes English-Only, English-Shared, Spanish-Only and Spanish-Shared. English-Only contains phonemes that are only found in the English dataset and Spanish-Only contains phonemes that are only found in the Spanish dataset. The phonemes found in the English-Only and Spanish-Only groups are then removed from a list of all the phonemes found by the Montreal Forced Aligner (MFA) across both datasets.\thoughts{do I need to make a bigger deal of the MFA stuff here? I have mentioned it in the methodology} A mean average of the phonemes in this list in the English dataset only is used for the English-Only group and the average of these phonemes found in the Spanish dataset only is used for the Spanish-Only group. Splitting the overlapping phonemes across the languages into groups allows us to see if there are any language-specific trends for the shared phonemes. 

In Figure \ref{ed_fig:phonemes_shared_not_shared} we first look at the English-Only group of phonemes, here we see a difference on \hl{ABS DIFFERENCE} between the phoneme accuracy recorded between the monolingual English model (960/0) and the Spanish monolingual model (0/920). While the accuracy does not consistently decrease with every step decreasing the amount of English data we do see an overall trend that the English-Only phonemes have the highest accuracy on the models with the most English data included in pretraining. For the Spanish-Only phoneme group we see the inverse trend with a difference of \hl{ABS DIFFERENCE} between the English monolingual and Spanish monolingual models, showing higher accuracy for Spanish-Only phonemes on the model only trained on Spanish pretraining data. Similarly, we see an increasing of accuracy as more Spanish data and less English data is introduced in pretraining. The behaviour of accuracy on phonemes that are unique with these two groups might be expected given that these phonemes do not exist in one or the other dataset, so to further examine the influence of pretraining we must analyse the English-Shared and Spanish-Shared groups. 

For the phoneme group English-Shared in Figure \ref{ed_fig:phonemes_shared_not_shared} there is an absolute difference of \hl{ABS DIFF} between the accuracy on the monolingual English model and the monolingual Spanish model. This group also exhibits the same behaviour as English-Only with accuracy increasing as more English data is introduced to the pretraining. Spanish-Only also exhibits the same pattern, having a change in accuracy of \hl{ABS DIFF} between the English and Spanish monolingual models and an increase in accuracy with models with more Spanish data in pretraining. This shows that even when phonemes exist in both languages they are not learned equally well and the models are learning more than discrete phoneme representations. 

Finally, we can note that in Figure \ref{ed_fig:phonemes_shared_not_shared} that the average phoneme accuracy for the English-Shared group is on every model higher than the average accuracy for the English-Only phoneme group. It is also true that the phoneme accuracy is higher on every model for the Spanish-Shared phoneme group when compared to the Spanish-Only phoneme group. This suggests that while shared phonemes are better recognised when they are spoken in a single language, phonemes that exist in both pretraining datasets still benefit from more examples in training.\thoughts{Do I need to mention the variance of the Spanish unique phonemes and how that's probably because there's so little data?}
...
Figure \ref{ed_fig:phonemes_shared_french_polish} shows the average phoneme accuracy for several different groups: phonemes that exist in both the French language and English or Spanish, Phonemes that exist in French but not in English or Spanish, then Phonemes that exist in Polish and also in English or Spanish and Phonemes that only exist in Polish but not in English or Spanish. Both languages were taken from the Multilingual LibriSpeech (MLS) dataset and phonemes found by using the Montreal Forced Aligner (MFA). 

In all models in Figure \ref{ed_fig:phonemes_shared_french_polish} the French language data has higher accuracy on phonemes that exist in both French and English or Spanish while having lower average accuracy on phonemes that are unique to French and not exist in either of the pretraining languages, English or Spanish. This pattern stays true with the accuracy for Polish data with average accuracy for phonemes that exist in both Polish and English or Spanish having a higher average accuracy when compared to phonemes that only exist in Polish and not in English or Spanish.

Throughout this section we have shown that phonemes that overlap between the English phoneme sets and the Spanish phoneme sets have the highest accuracy\thoughts{I could include this phoneme group in Figure \ref{ed_fig:phonemes_shared_french_polish}?} due to them existing in all wav2vec 2.0 models we have pretrained. Phonemes that are unique to English have significant accuracy increases when tested on models with high ratios of English data in pretraining. The inverse is true for Spanish with the highest accuracy phonemes that are unique to Spanish being recorded on the model trained only on Spanish pretraining data. Figure \ref{ed_fig:phonemes_shared_french_polish} shows that for both the French language and Polish language phonemes that exist in either language as well as English or Spanish always achieve higher accuracy than phonemes that are unique to each language. We have seen in Section \ref{ed_subsec:gain_across_languages} that the best approach for low error transcription is to pretrain the model on the language you are finetuning or a language closely related to it. In this Section however we have seen that there may be a middle ground whereby if we carefully select pretraining data to contain phonemes relevant to the downstream language we may be able to achieve higher accuracy. To further dissect this idea we can evaluate performance across different phoneme groups to measure whether language is more or less dependant on specific groups of phonemes. %\thoughts{This section could probably be moved to discussion LOOK HERE ED!!!}

\iffalse
\hl{THIS WHOLE SECTION WILL NOW NEED RE-WRITING AS I HAVE CHANGED THE GRAPH FROM A LINEGRAPH TO A BARCHART} Figure \ref{ed_fig:phonemes_shared_plus_french} shows the mean phoneme accuracy across all phonemes for four languages: English, Spanish, French and Polish. Each language is split into two phonemes groups: phonemes shared by English and Spanish and phonemes unique to that language. These groups were chosen in order to test whether there is any significant difference between phonemes that exist in the training data of all of the models and phonemes that do not exist in the pretraining

Figure \ref{ed_fig:phonemes_shared_plus_french} reinforces that the best recognised phonemes are those shared by both of the training languages. 
The left most bar (purple) for each model shows the average accuracy of phonemes that appear in both English and Spanish when measured on data from English only. 
\fi
\iffalse %%%% REMOVED
Across all models this group has the highest accuracy, it also shows little preference towards models with more or less English or Spanish data. The next line, "Shared Phonemes (En+SP) tested in Fr", shows the same phoneme group phonemes shared in English and Spanish, but shows the average accuracy on each model when tested on French spoken language data. This lines shows higher accuracy on the English monolingual model when compared to the Spanish monolingual model but does not show this pattern consistently across the mixed data models. The accuracy is slightly higher for the model trained on 450 hours of both languages than either the model trained on 600 hours of English and 300 hours of Spanish data or the model trained on 300 hours of English and 600 hours of Spanish data, for example.
\fi 

\iffalse %%%%%%%%%  COME BACK TO THIS THERE MIGHT BE SOME USEFUL STUFF HERE
Finally, we have two lines calculated as the average accuracy for each model on phonemes unique to English, "English-Only Tested in Fr", and phonemes unique to French, "French-Only tested in Fr", when spoken in French from the MLS French \cite{MLS}. Phonemes unique to Spanish but spoken in French are not included as there was no data available in MLS French for these phonemes. The accuracy of phonemes only found in English \hl{I NEED TO FIND ABSOLUTE VARIANCE VALUES HERE} is marginally higher on the English monolingual model than the Spanish monolingual model, with a difference of \hl{exact acc difference} which is significantly less than the same group in Figure \ref{ed_fig:phonemes_shared_not_shared} when tested on English data which has a difference of \hl{ABS acc DIFF}. The phonemes that are unique to French when tested on French data have an even smaller accuracy difference of \hl{abs ACC diff}. This shows that when the phonemes are extracted from French spoken data the data balance is less impactful. What is however noticeable is that both of the groups with phonemes unique to each language are lower in accuracy for every model than the accuracy at each model in both of the shared phoneme groups. \thoughts{Is it worth talking in terms of variance between the accuracy found on each model?}

French language data was not included in the pretraining data so it is unsurprising that phonemes unique to French show low relative accuracy, however it is notable that phonemes that exist in both English and Spanish show higher accuracy even when spoken in French. Figure \ref{ed_fig:phonemes_shared_not_shared} shows that monolingual models have the highest accuracy on phonemes in this shared group when spoken in the language they were pretrained on. Figure \ref{ed_fig:phonemes_shared_plus_french} however, shows that mixing data in pretraining does produce higher accuracy on individual phonemes that are shared across the languages included in pretraining even when they are spoken in a language not included in pretraining. This may explain why related languages were found to have lower Character Error Rate (CER) in section \ref{sec:gain_across_languages}, it also suggests that carefully choosing your pretraining data based on the phonetic content could improve the accuracy or error rates when fine-tuning to new unseen languages. \thoughts{I could reference that paper that does this phoneme recognition here OR I could move these last sentences to the discussion section}
\fi
%\newpage
\subsection{Phoneme Class Accuracy Trends}\label{ed_sec:phonemes_classes}
\iffalse
\begin{itemize}
    \item I may need to talk about which classes have more overlap when talking about which ones have high or low accuracy
    \item Ordering of graphs: English, Spanish and then French and then big combined graphs  over 2 pages
    \item I may need to chop the analysis up into subsections for each phoneme group
    %\item 
\end{itemize}
\fi
\iffalse
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{5_chapter5/figures/Phoneme_by_category_acc_layer_8_only_english.pdf}
    \caption{Phoneme accuracy taken at layer 8 of each of our models for English data, the phonemes are split into sub classes. \hl{both the graph and description will need updating}}
    \label{ed_fig:Phonemes_Groups_English}
\end{figure}
\fi

To further test this theory we can break down our phoneme data into sub-groups, this will allow us to see whether the language or the type of phoneme is more important for recognition.
\iffalse
\begin{itemize}
    \item I need to collect some data on each of these phoneme groups and put them into a table
    \item Number of phonemes in each group per language 
    \item Absolute difference in accuracy per group, although I kind of have this with the bar chart 
    \item relative accuracy change?
\end{itemize}
...
\iffalse
%%%%%% Original Table before language split
\begin{longtable}[!b]{>{\raggedright\arraybackslash}p{3cm} p{12cm}}
\toprule
\textbf{Phoneme Group} & \textbf{Phonemes} \\
\midrule
Vowel & \textipa{a, a:, e, e:, i, i:, o, o:, u, u:, {\ae}, 5, A, A:, 6, 6:, O, @, E, E:, 3, 3:, \textrhookschwa, I,  0, 0:, U, y, \o, \oe, \~{A}, \~{O}, \~{E}, \textrhookrevepsilon} \\
Diphthong & \textipa{aj, aw, ej, ow, Oj, @w} \\
Stop & \textipa{b, b\super j, c, c\super h, c\super w, d, d\super j, \|[{d}, g,  g\super w, \textg, k, k\super h, k\super w,  p, p\super h, p\super j, p\super w, t, t\super h, t\super j,  t\super w, \|[{t}, \:d, \textObardotlessj, \textObardotlessj\super w, \textObardotlessj J, \:t, \:t\super j, \:t\super w, P} \\
Nasal & \textipa{m, m \super j, \textsyllabic{m}, n, \textsyllabic{n} N, M, \textltailn}\\
Fricative & \textipa{f, f\super j, h, s, v, v\super j, x, z, \c{c}, D, G, S, Z, J, B, T, J, K} \\
Affricate & \textipa{dZ, tS, ts} \\
Approximant & \textipa{j, l, \textltilde, \textsyllabic{\textltilde} w, \*r, L, 4, V} \\
Closure & sil (silence) \\
Tap \& Trill & \textipa{r, R, R\super j, \~{R}} \\
\bottomrule
\caption{Table of phonemes included in each group analysed in this section \hl{I can probably reduce the size of the table by having a note that says that all stresses and modifications to phonemes are also included in each group}}
\label{ed_tab:Phonemes_Groups_English}
\end{longtable}
\fi
\iffalse
\begin{itemize}
    \item We need to explain each of the phoneme groups
    \item All groups include phonemes taken from both  of the phoneme sets included in the english\_us and spanish\_spain Montreal Forced Aligner (MFA) pronunciation dictionaries \cite{mcauliffe2017montreal}  
    \item The groups are partially based around the groups in \cite{english2022domain} and then expanded on as below
    \item Vowels take every vowel sound from both languages and contains \hl{X} unique phonemes as well as all \hl{word for modifications!!}
    \item Diphthongs are separated out from vowels due to the articulation between phonemes. Since articulation may be important when judging the accuracy between different languages
    \item Diphthongs are only available in English
    \item \hl{I need to find justifications for Nasal, Fricative, Affricate and Approximants or I could just}
    \item Closure represents periods of silence which MFA recognises as the sil token, we expect little difference between languages
    \item Tap \& Trill are a subset of liquids which is itself a subset of approximants, the Tap\& Trill were separated into their own group because they only exist in the Spanish Language and then the remaining Spanish Liquids were included in Approximant
\end{itemize}
\fi
To further test this theory we can break down our phoneme data into sub-groups, this will allow us to explore whether the language or the phoneme groups are more important for recognition. Table \ref{tab:phoneme_groups_by_language} shows the breakdown of phonemes in each phoneme group, the phonemes are selected from all of the phonemes in the Montreal forced aligner dictionaries for English and Spanish. All phonemes modifications, such as \textipa{a:} or \textipa{p\super j,}, are also included in the group with their unmodified phonemes. The full list of phoneme tokens used in each language can be found in Appendix \ref{app_sec:chapter_5_appendix} Table \ref{app_tab:phoneme_class_acc_with_diff}

The groups of phonemes chosen follow the same groupings as found in English et al. \cite{english2022domain} with some exceptions. All vowels were grouped together with any stessers also included. Diphthongs, that exist in English but not Spanish, were separated into their own group, this was due to the possibility that coarticulation between the phonemes may be relevant to the pretraining languages. Closure simply includes the sil token which denotes a lack of any sound for a short period of time. Finally the phonemes \textipa{r} and  \textipa{R} were separated into their own group, while they could have been included in the Approximant group both \textipa{r} and \textipa{R} are unique to Spanish so might be affected by pretraining data more than other approximants.

Figure \ref{ed_fig:Phonemes_Groups_English_linegraph} shows a breakdown of the average accuracy of different phoneme classes validation data from the English LibriSpeech dataset, the Multilingual LibriSpeech (MLS) Spanish subset and the MLS French subset. The groups show accuracy from layer 7 of each of the 7 pretrained models for the nine different phoneme groups described in Table \ref{ed_fig:Phonemes_Groups_English_linegraph}. Figure \ref{ed_fig:Phonemes_Groups_English_barchart} shows the difference in accuracy of each phoneme group for each language when taken between the accuracy of the monolingual English model trained only on LibriSpeech data and the monolingual Spanish model trained only on the Multilingual LibriSpeech (MLS) Spanish subset. This figure describes the difference in accuracy between the biases introduced with English pretraining vs Spanish pretraining.\thoughts{do I need to have a short equation here? If so I would need one for figure \ref{ed_fig:multilingual_barchart} as well} 

\iffalse
\begin{itemize}
    \item \textbf{Vowels}: Spanish has the highest accuracy on all models, Spanish also has the fewest phonemes in this group so maybe this has something to do with it. Can we also talk about Spanish vowels being more distinct? this may be hard to reference
    \item \textbf{Diphthongs}: Only Exist in English, clearly have the highest accuracy on the English monolingual
    \item \textbf{Approximant}: Another clear class where the phonemes in English and Spanish have clear preferences to their monlinguial models
    \item \textbf{Approximant}: In this case it is not clear which model french prefers and while polish has an english bias it is not clear which language helps it more since the 2 highest accuracy models are the 800/100 model and the 300/600 model and the lowest being the 600/300 and the Spanish monolingual
    \item \textbf{Nasals}: All languages including Spanish have a clear preference towards English 
    \item \textbf{Nasals}: for English the validation total number of examples for \textipa{m, n, N, M} (the shared phonemes) is 19056 whereas the number of examples in Spanish is 35245
    \item \textbf{Nasals}: for English the training total number of examples for \textipa{m, n, N, M} (the shared phonemes) is 27774 whereas the number of examples in Spanish is 15487
    \item \textbf{Nasals}: Since the Spanish model with the highest accuracy on nasals is the 100/800 maybe this is not so clear cut
    \item \textbf{Tap \& Trill}: Does not exist in French, has relatively few occurrences in English. Spanish has the highest accuracy on all models, polish might be reduced due to the \textipa{r\super j} token only having 12 occurances
    \item \textbf{Tap \& Trill}: Also interesting that Polish has a slight preference for the Spanish model, could this be because it contains the Trill (\textipa{r}) whereas English only has the Tap (\textipa{R})?
    \item \textbf{Stops}: 
    \item \textbf{Affricate}: This groups shows preferences towards the English model for all languages Spanish only has one token in this group and the lowest accuracy is at 300/600 and the highest on 450/450 this phoneme only has 391 examples in training when compared to English 3183, possibly the Spanish probes are not as well equipped to recognise this phoneme 
    \item \textbf{Fricative}: Spanish prefers SPanish English prefers English f5rench and polish prefer English, it is not entirely clear why
    \item \textbf{Closure}: This is just the sil token denoting silence, it is high accuracy for all languages and the preferences are all very low, silence seems to be mostly language independant
    \item \textbf{Takeaway}: \hl{I may need to do a phoneme breakdown of each of the training sets}
\end{itemize}
\thoughts{Do I need to run the phoneme count on the training data? If so do I need to do it for all models? Maybe I could do this for phoneme groups rather than specific phonemes}
\fi
In the vowel plot on Figure \ref{ed_fig:Phonemes_Groups_English_linegraph} we see that Spanish vowels have the highest accuracy across all models being above 80\% for all models except the English monolingual model (960 / 0). Notably, Spanish has the fewest total individual vowels of all of the languages as seen in Table \ref{app_tab:phoneme_groups_by_language}, Spanish has no variants to the phonemes in the vowels which can occur less frequently in both training and validation data and therefore lower accuracy the average accuracy. It is also notable that the Polish vowels have the highest accuracy on the balanced mixed model (450 / 450), this could imply that when the testing language is unrelated to the pretaining languages individual phonemes are more impactful than the relationships between phonemes. Figure \ref{ed_fig:Phonemes_Groups_English_barchart} confirms that Polish vowels have the least difference between accuracy on each of the monolingual models, whilst English is the most affected. 
...
For Stops we see the pattern shown in previous sections with English phonemes having higher accuracy on the English monolignual model when compared to the Spanish monolingual model with a difference of 19.3\%. The Inverse is then true for Spanish, althought to a lesser degree, with Spanish spoken phonemes having higher accuracy on the Spnaish monolingual model and lower on the English model although this time with a difference of 1.3\%. Interestingly the highest accuracy for Spanish is found on the balanced model (450 / 450), siggesting SPanish is benefitting from balanced monolingual pretraining. French and Polish both have higher accuracy on the English monolingual model when compared to the Spanish monolingual model. This affect may be due to English having a much larger variation of phonemes in this group when compared to Spanish, this may be beneficial for accuracy for phonemes in this group. In this group French nasal phonemes have the highest accuracy across all models except the 300 / 600 model. This shows that not all phoneme groups are recognised equally in each language.

For Affricates Polish shows the least variation with only 1.4\% difference between accuracy on the English and Spanish monolingual models. English has the highest accuracy on the English monolingual model and the lowest on the SPanish monolingual model showing preference to pretraining on the same language. Spanish However has higher accuracy on the English monolingual model when compared to the Spanish monolingual model. The highest recorded accuracy is on the 450 / 450 model while the lowest is on the 300 / 600 model, giving Spanish the most variation of the languages in this category. As seen in Table \ref{app_tab:phoneme_groups_by_language} Spanish only contains one phoneme in this category, \textipa{tS} and there are only 391 examples of the phoneme in the training data for the probes as seen in Table \ref{app_tab:phoneme_counts_per_language_train_10h_3}, when compared to phonemes that often have more than 10,000 examples. Training probes to recognise a single phoneme, with relatively few examples may contribute to the variation in this case.

For fricatives again we see English again having higher accuracy on the English monolingual model when compared to the accuracy on the Spanish monolingual model. Spanish fricatives then have the highest accuracy on the Spanish monolingual model and the lowest on the English monolingual model. Both French and Polish have higher accuracy across all models than either English or Spanish, which is not true of any other phoneme group.
...
Overall the findings in this section show that phoneme accuracy is not consistent across different categories of phonemes. As seen in nasals and affricates the concept that pretraining on the same or a related language always increases accuracy is not true for these groups and in the case of Spanish stops and nasals, a balanced mix of pretraining gives the highest accuracy for these phonemes. In most cases however phonemes spoken in English do have higher accuracy on the models with the most English in pretraining and phonemes spoken in Spanish have the highest accuracy on the models with the most Spanish data in pretraining. 
...
%%%%% DOn't delete this re-read and see what may be useful here
\iffalse
\begin{itemize}
    \item A talking point can be about the phonemes having higher general accuracy when they are shared
    \item is this because they are individually better recognised or is it simply because there are more examples, even just for the probes
    \item are they higher because there are more of them to bring the average up
    \item The question is is it better to have balanced pretraining data where a large pool of shared phonemes have overall better accuracy or is it better to have 
    \item TO THINK ABOUT: Do I need an extra experiment to justify this?
    \item on the other hand I could argue that the language with the most phonemes could just be the best idea (this would be english)
    \item Adding languages will not increase the shared group so what then would be the new experiment, because surely by adding more languages you are lowering the accuracy of all the excluded phonemes
    \item could the last experiment be fine-tuning to datatang spanish
    \item I probably need to come back to this once I've done some more analysis on exactly how the phonemes are behaving?
    \item the more languages you add to pretraining the smaller you're shared pool is becoming so does that mean that more languages the more phonemes that are worse recognised? You could also argue that some might exist in a certain language but might not be that common so you would increase accuracy then.
    \item do I need to prove this premise that not shared phonemes have lower accuracy the more languages you add in, given that I have only pretrained with 2 languages?
    \item what does it add if I did prove it? another questions would be are phonemes that are shared more accurate than just training a monolignual model, this seems unlikely given the data.
    \item for this to be a worthwhile experiment we would have to have found that phonemes that are shared have either the highest accuracy on the balanced model or at elast comparable, but this isn't really true, the monolignual models always outperform
    \item the experiment we could do would be to create a phoneme dictionary that includes all of the phonemes of a third language (i.e. French) and then train on those languages
    \item However, realistically, this is english and spanish or at least they are good candidates already.
    \item If I were to make a model that was trained on data from say 9 languages we could compare phoneme accuracy all the shared languages and expect to see the small group of shared phonemes be the highest, but then it would have to be higher than that same group of phonemes on a monolingual group
\end{itemize}
\fi
%%%%% DOn't delete this re-read and see what may be useful here
\iffalse
\begin{itemize}
    \item \hl{\textbf{These are not in order but will correlate to the previous results section and what should be dscussed here }}
    \item \textbf{ASR Finetuning}: First we pretrain multiple models on different ratios of different languages. While the relationships between finetuning languages and pretraining languages has been explored the concept of balancing your pretraining data has not been thoroughly explored
    \item \textbf{ASR Finetuning}: Our first experiment shows the relationship between pretraining data and finetuning data with english and spanish clearly preferring their own pretrained models
    \item \textbf{Multilingual ASR Finetuning}: Our second experiment shows that previous work that shows that previous work that says that finetuning to languages that are related to the pretraining language have lower CER when there is more of that language in pretraining. But why?
    \item \textbf{Multilingual ASR Finetuning} the advantage of self-supervised pretraining is that we can break down the ASR training into two stages: phonetic learning and semantic learning. Since we know that pretraining affects the finetuned accuracy, we can assess the SSL pretraining phoneme accuracy to see what it is learning. Since the pretrained phonetic information does not change between pretraining and finetuning it is important to know what information is baked into the model.
    \item \textbf{IOUs}: We could mention that IOUs were done as with the previous chapter, but not go into much detail \hl{this is probably the way}
    \item \textbf{IOUs}: The results from the IOUs correlate to the previous chapter and show that wav2vec, like XLS-R, is allocating different areas of the model to different languages
    \item \textbf{IOUs}: It should be noted that XLS-R is much larger and can therefore afford the space within the model to dedicate to a single language \cite{some_sort_of_citation_for_this!!}
    \item \textbf{IOUs}: whereas wav2vec 2.0 is smaller and we see a larger impact on the CER for downstream 
    \item \textbf{Probing for shared phonemes} Discussion can first talk about monolingual models being better for accuracy on that language
    \item \textbf{Probing for shared phonemes} Then the next talking point can say that if you could choose your data to cover the phonetic content of a low-resource language you may get better results \hl{that paper that does this with catalan and stuff kind of did this but on a small scale with relatively few languages.}
    \item \textbf{Probing for shared phonemes} firstly phonemes in the languages of the pretraining bias towards their languages
    \item \textbf{Probing for shared phonemes} phonemes spoken in languages not in the pretraining are much more neutral but the phonemes in the pretraining are higher in accuracy
    \item \textbf{Probing for shared phonemes} \hl{Do I need to run this on another languge that isn't french?}
    \item \textbf{Probing for phonemes groups} I can talk about the different phoneme groups and their accuracy
    \item \textbf{Probing for shared phonemes} I need to mention the increased number of variations in English and how this many have affected my accuracy, although it shouldn't be important to the findings
    \item \textbf{Probing for shared phonemes} The low number of examples for English Tap and Trill
    \item \textbf{Probing for shared phonemes} The low number of examples for Spanish Affricate
    \item \textbf{Probing for shared phonemes} Different groups of phonemes react differently to pretraining, monolingual pretraining is not consistently beneficial across all groups
    \item \textbf{Probing for shared phonemes} Both French and polish trend towards English more often however English is a more varied language with a larger number of phonemes and the variation in accuracy is much less when compared to English or Spanish
    \item \textbf{Probing for shared phonemes} English does better more often, could this be because it has a wider range of phonemes (90+ compared to ~30 in spanish)
    \item \textbf{Probing for shared phonemes} We want to see how many examples of each phoneme group exist in pretraining data vs how many exist in the test and training data for each of the languages.
    \item \textbf{Probing for shared phonemes} Finally we want to say that for most cases monolingual pretraining is more likely to be beneficial, but could it be that if pretraining data was designed and picked carefully from unrelated languages if you had similar ratios of phonemes in each category or unique phonemes could you get higher accuracy and lower CER withthe right pretraining? 
    \item \textbf{Probing for shared phonemes} somewhere I should be noting as well that both french and polish lean towards English, maybe I could do an analysis of the overlapping phonemes to show that they both have more in common with English
    \item \textbf{Probing for shared phonemes} If the previous point is true then it could be said that a more phonetically diverse training set is beneficial for downstream training
    \item \textbf{Overall conclusions} Pretraining is impactful on downstream performance, pretraining on the right data affects performance later on and this is hard to change after an SSL ASR model has been trained on a large unlabelled dataset.
    \item \textbf{Overall conclusions} Monolingual training shows higher accuracy for the same language and langauge related to it, however this is an oversimplified conclusion
    \item \textbf{Overall conclusions} Since SSL ASR learns phonemes we can concetrate on mixing our pretraining data at a phonetic level
    \item \textbf{Overall conclusions} We find that individual phonemes that are shared by both training sets do best on models with both sets of data and unique phonemes do worse the less of that languages data is in pretraining. Phonemes spoken in unrelated languages but that do exist in the pretraining data (even when spoken in another language) perform better than phonemes not in pretraining in an unrelated language
    \item \textbf{Overall conclusions} Different phoneme groups have different preferences that ar not necessarily related to the relationship to the language but rather to the language's relation to the phoneme groups.
    \item \textbf{Overall conclusions} If pretraining data was selected carefully to match the downstream data, could it perform well regardless of the languages included? Is the higher performance on related languages only higher because those langauges have similar phonetic makeup?
    \item \textbf{Future Work} talk about the potential for choosing a broad range of phonemes or creating a list of phonemes for a languages and using this to create a model that generalises well
    \textbf{Notes} Read and include reference to the interspeech paper on dutch ssl asr
\end{itemize}

\begin{itemize}
\item ----------------------------------------------------------------------------------------------------
    \item \hl{This section should include a reference to my distillation experiments OR maybe I could include at the end of the last section??}
    %\item \hl{FILLER POINT}
    %\item \hl{FILLER POINT}
    %\item \hl{FILLER POINT}
    %\item \hl{FILLER POINT}
    %\item \hl{FILLER POINT}
