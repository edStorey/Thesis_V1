...
\end{table}
\hl{is this correct? the values seem very low for english??}
...
\subsection{Language-Membership Phoneme Groups}\label{ed_subsec:lang_membership_groups}

Our first set of probing experiments looks at how pretraining language balance affects phoneme recognition. We look at phonemes that English and Spanish share, and phonemes that are specific to one of the two languages. We obtain these groups by comparing the phoneme symbol lists defined in the English and Spanish MFA pronunciation dictionaries. First, we calculate the intersection of the two lists and treat this intersection as the set of phonemes shared by both languages. Symbols that are in the English list but not in the Spanish list are treated as English‑unique phonemes. Then, symbols that are in the Spanish list but not in the English list are treated as Spanish‑unique phonemes. The resulting groups are:
...
\begin{table}[h]
    ...
    \label{tab:phoneme_group_sizes_en_es}
\end{table}
...
\hl{THIS IS THE NEXT SECTION TO REWRITE}

Table \ref{ed_tab:phoneme_group_sizes} shows the number of phonemes in each group and, for the shared groups, how many of those phonemes occur in each evaluation language. For each pretrained model and each phoneme group, we calculate the accuracy in two steps. First, we get the probe accuracy for each individual phoneme in the group. Then we take the mean of these phoneme accuracies to obtain the average accuracy for that group. For English and Spanish, we evaluate both their shared and language-unique groups. For French and Polish, we compare the shared English Spanish group with the French-unique and Polish-unique groups.

The shared English Spanish inventory contains many more phonemes than, for example, the Spanish-unique group, and the English MFA dictionary in general has a larger phoneme inventory than the other languages. This means that group accuracies are averaged over different numbers of phonemes, and small groups (such as Spanish-unique) may be more affected by noise or outlier results.\thoughts{I will need to rewrite this when I have the numbers in the above graph because I don't know how the absolute and potential number of phonemes interact}

For French and Polish, we do not include “English-unique” or “Spanish-unique” groups in the analysis. None of the Spanish-unique phonemes appear in the French or Polish inventories, so these segments cannot be evaluated in those languages. English-unique phonemes do appear in French and Polish, so it would be possible to define “English-unique in French” and “English-unique in Polish” groups to study transfer of English-specific segments. In this chapter, we focus instead on two contrasts that are more directly tied to our pretraining setup: (i)\thoughts{I don't use (i) or (ii)} phonemes that are shared by the English–Spanish pretraining pair and reused in French and Polish, and (ii) phonemes that are unique to French or Polish and absent from the English and Spanish pretraining inventories.\thoughts{either I need to get rid of this paragraph or I need to re do the experiments with all English and Spanish phonemes rather than just the intersection phonemes}
