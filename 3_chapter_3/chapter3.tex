...
\section{Discussion}
This is a summary of cormac's paper \cite{english2022domain} that coul dbe relevant to this chapter:

\hl{Their analyses indicate that contextualised representations (after the transformer) are substantially better for phonetic classification than purely convolutional features, confirming that the model’s context modelling is crucial for capturing fine-grained phonetic structure.}\thoughts{this may be more useful for the discussion in the first chapter, just putting it here to remind myself}
